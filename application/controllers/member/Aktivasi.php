...
class Aktivasi extends CI_Controller {
	...
	function index(){
		...
		$data['title'] = "Data Member Aktivasi";
	...
	$data['list'] = "member/aktivasi/data_list";
		$id = 2;
/*
	...
	//		$page = $_POST['length']/$limit;
		...
		CURLOPT_URL => URL_API."admin/pending_member?perpage=$limit&page=$page",
		...
		foreach($dat as $l) {
			...
			$edit =  "<a class='btn btn-success btn-xs' id='openkegiatan'  data-toggle='modal' data-target='#showmodalkegiatan' data-header='Konfirmasi' href='#' data-id='$id' data-href='".base_url("member/aktivasi/approve?id=$id")."'>Approve</a>";
			$delete = "<a class='btn btn-danger btn-xs' id='deletekegiatan'  href='#' data-id='$id' data-href='".base_url("api/cms/reject?id=$id")."'>Reject</a>";
